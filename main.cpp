...
int main() {
    ...
    while (true) {
        cout << "\n--- 개인 일정 관리 시스템 ---" << endl;
        cout << "1. 추가  2. 전체 보기  3. 삭제  4. 정렬  5. 검색  6. 저장  7. 테스트  8. 상태변경  0. 종료" << endl;
        cout << "선택: ";
        cin >> choice;
    ...
    break;
        ...
        try {
            if (choice == 1) {
               ...
               int idx, s;
                cout << "변경할 일정 번호(전체 보기의 번호): "; cin >> idx;
             cout << "새 상태(0:미완료, 1:진행중, 2:완료): "; cin >> s;
             mySched.updateStatus(idx, s);
             cout << "상태가 변경되었습니다." << endl;
}
        } catch (exception& e) {
